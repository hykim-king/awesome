...
/* ===== 헤더 상단 바 ===== */
#header{
  background: rgba(2, 20, 71, 0.5) !important; 
}
  ...
  <div class="navbar">
    ...
    <div class="nav-right">
      <c:if test="${not empty sessionScope.loginUser}">
        <span class="user-id">${sessionScope.loginUser.nickNm}님 안녕하세요</span>
      </c:if>
      <div class="user-menu">
                    ...
                    <c:if test="${st.index lt 3}">
                        ...
                        <c:choose>
                            <c:when test="${idx1 ne -1}">
                                <c:set var="kwClean"
                                    value="${fn:trim(fn:substring(kwRaw, 0, idx1))}" />
                            </c:when>
                            <c:when test="${idx2 ne -1}">
                                <c:set var="kwClean"
                                    value="${fn:trim(fn:substring(kwRaw, 0, idx2))}" />
                            </c:when>
                            <c:otherwise>
                        ...
                        <c:url var="kwUrl" value="${k.link}">
                            ...
                            <c:param name="searchDiv" value="10" />
                            <!-- 제목 검색 -->
                            <c:param name="searchWord" value="${kwClean}" />
                            <!-- 괄호 앞부분만 -->
                        </c:url>

                        <a class="keyword" href="${kwUrl}"> <c:out value="${kwRaw}" />
                        </a>
                    </c:if>
                </c:forEach>
            </div>
        ...
        <div class="section-box">
            ...
            <ul class="popular-grid">
                <c:forEach var="item" items="${popularArticles}">
                    <li class="popular-card"><a href="${item.url}" target="_blank">

                            <!-- 아이콘 영역 -->
                            <div class="icon">
                                <c:choose>
                                    <c:when test="${item.category == 10}">
                                        <img src="${ctx}/resources/file/politics.png" alt="정치" />
                                    </c:when>
                                    <c:when test="${item.category == 20}">
                                        <img src="${ctx}/resources/file/002.png" alt="경제" />
                                    </c:when>
                                    <c:when test="${item.category == 30}">
                                        <img src="${ctx}/resources/file/003.png" alt="사회/문화" />
                                    </c:when>
                                    <c:when test="${item.category == 40}">
                                        <img src="${ctx}/resources/file/004.png" alt="스포츠" />
                                    </c:when>
                                    <c:when test="${item.category == 50}">
                                        <img src="${ctx}/resources/file/005.png" alt="연예" />
                                    </c:when>
                                    <c:when test="${item.category == 60}">
                                        <img src="${ctx}/resources/file/006.png" alt="IT/과학" />
                                    </c:when>
                                            </c:choose>
                            </div> <!-- 기사 제목 -->
                            <div class="title">
                    ...
                    <c:otherwise>
                        ...
                        <div class="top-feature">
                            ...
                            <div class="top-headline">
                                ...
                                <h3 class="top-title">
                                    <a href="${visitMain}" target="_blank"
                                        rel="noopener noreferrer"> <c:out value="${mainRec.title}" />
                                    </a>
                                </h3>
                            </div>
                            <p class="top-summary">
                                <c:out value="${mainRec.summary}" />
                            </p>
                        </div>
                        ...
                        <details class="more-recs">
                            ...
                            <div class="rec-cards">
                                <c:forEach var="it" items="${personalRecs}" begin="1" end="3">
                                    ...
                                    <a class="rec-card" href="${visitSub}" target="_blank"
                                        rel="noopener noreferrer"> <span class="press-badge"><c:out
                                                value="${it.press}" /></span> <strong class="title"><c:out
                                                value="${it.title}" /></strong> <!-- 요약: 길면 줄임 -->
                                        <p class="summary">
