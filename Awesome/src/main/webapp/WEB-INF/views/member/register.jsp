    ...
    <div class="wrap">
      ...
      <form id="joinForm" method="post" action="<c:url value='/member/register.do'/>">
        ...
        <div class="group">
          ...
          <div class="row">
            <!-- name 추가 -->
            <input id="mailAddr" name="mailAddr" type="email" required placeholder="example@domain.com"/>
            <button id="btnSendCode" type="button" class="btn">인증하기</button>
          </div>
          <small id="emailHint" class="hint">메일에서 인증 링크 또는 코드를 확인해주세요.</small>
        ...
        <div class="group">
          ...
          <div class="row">
            <input id="emailCode" type="text" maxlength="6" placeholder="인증번호 6자리를 입력해주세요"/>
        ...
        <button id="btnSubmit" class="btn" type="submit">회원가입 완료</button>
      </form>
    </div>
  </main>
...
<script>
  ...
  btnSend?.addEventListener('click', async()=>{
    ...
    if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){
      ...
      emailHint && (emailHint.style.display='none');
      return;
    }
    ...
    try{
      ...
      if(data.ok){
        let left=60;
        btnSend.disabled=true; btnSend.textContent='재전송('+left+'s)';
        const timer=setInterval(()=>{
          left--; btnSend.textContent='재전송('+left+'s)';
          if(left<=0){ clearInterval(timer); btnSend.disabled=false; btnSend.textContent='인증하기'; }
        },1000);
      }
    }catch(e){ msg.className='msg err'; msg.textContent='요청 실패: '+e.message; }
  });
  ...
  btnVer?.addEventListener('click', async()=>{
    ...
    try{
      const data=await postForm(CP+'/member/email/verifyCode.do',{mailAddr:email, code});
      msg.className='msg '+(data.ok?'ok':'err');
      msg.textContent=data.message || (data.ok?'이메일 인증 완료!':'코드 확인 실패');
      if(data.ok){ f.emailOk.value='Y'; f.mail.readOnly=true; btnSend.disabled=true; f.code.readOnly=true; }
    }catch(e){ msg.className='msg err'; msg.textContent='요청 실패: '+e.message; }
  });
  ...
  (function(){
    ...
    agreeAll?.addEventListener('change', e=>{
      agrees.forEach(cb=>cb.checked=e.target.checked); refresh();
    });
    agrees.forEach(cb=>cb.addEventListener('change', refresh));
    refresh();

    document.getElementById('joinForm')?.addEventListener('submit', e=>{
      if(f.pwd.value!==f.pwd2.value){ e.preventDefault(); alert('비밀번호가 일치하지 않습니다.'); return; }
      if(!requireds.every(cb=>cb.checked)){ e.preventDefault(); alert('필수 약관에 동의해 주세요.'); return; }
      if(f.emailOk.value!=='Y'){ e.preventDefault(); alert('이메일 인증을 완료하세요.'); return; }
    });
  })();
